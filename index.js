...
const token = '7775701776:AAFcZ1OXS8ivwYpzlG1xko8V_ucFJPmbbUU';
...
const startGame = async chatId => {
	await bot.sendMessage(
		chatId,
		"Kompyuter 0 dan 9 gacham son o'yladi, siz usha sonni topishga xarakat qiling."
	);
	...
	bot.on('message', async msg => {
		...
		if (text === '/info') {
			await bot.sendSticker(
				chatId,
				'https://sl.combot.org/utyaduck/webp/16xf09fa494.webp'
			);
		...
		if (text === '/help') {
			return bot.sendMessage(
				chatId,
				`siz botimizda error topgan bo'lsangiz adminimizga yozing +998-93-885-40-40`
			);
		}
	});
